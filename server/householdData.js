...
router.post('/saveData', async (req, res) => {
  try {
    ...
    const householdCommonResult = await query(householdCommonQuery.text, householdCommonQuery.values);
    ...
    for (const member of familyFormData) {
      ...
      await query(familyMembersQuery.text, familyMembersQuery.values);
    }
    ...
    await query(recommendationQuery.text, recommendationQuery.values);
...
router.get("/entries", async (req, res) => {
  ...
  try {
    const result = await query(`SELECT * FROM household_common WHERE user_id = $1`, [userId]);
    res.status(200).json(result.rows);
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Server Error");
...
router.get("/recommendations", async (req, res) => {
  ...
  try {
    const result = await query(`SELECT * FROM recommendations WHERE user_id = $1`, [userId]);
    res.status(200).json(result.rows);
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Server Error");
...
router.get("/familyEntries/:entryId", async (req, res) => {
  ...
  try {
    const result = await query("SELECT * FROM family_members WHERE household_common_id = $1", [entryId]);
    res.status(200).json(result.rows);
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Server Error");
